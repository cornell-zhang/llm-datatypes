...
import neural_compressor.adaptor.torch_utils.weight_only as weight_only
...
class GPTQ:
    ...
    def fasterquant(self, W, blocksize=128, dtype=None, percdamp=0.01, groupsize=-1, act_order=False):
        ...
        if not self.quantizer.ready():
            self.quantizer.find_params(W, weight=True, dtype=dtype)
        ...
        for i1 in range(0, self.columns, blocksize):
            ...
            for i in range(count):  # within a block, channel wise
                ...
                if groupsize != -1:
                    if (i1 + i) % groupsize == 0:
                        self.quantizer.find_params(W[:, (i1 + i) : (i1 + i + groupsize)], weight=True, dtype=dtype)
                        scale.append(self.quantizer.scale)
                        zero.append(self.quantizer.zero)
                if dtype in weight_only.FLOAT_MAPPING:
                    q = weight_only.quantize_4bit(w.unsqueeze(1), data_type=dtype, scale=self.quantizer.scale).flatten()
                else:
...
class Quantizer(nn.Module):
    ...
    def find_params(self, x, weight=False, dtype='int4'):
        if dtype in weight_only.FLOAT_MAPPING:
            dtype_values = weight_only.FLOAT_MAPPING[dtype]
            max_dtype = torch.max(torch.abs(torch.tensor(dtype_values)))
            scale = x.abs().max(dim=1)[0] / max_dtype
