...
class LookupLinear(torch.nn.Module):
    def __init__(
        self,
        orig_layer,
        num_bits=4,
        ...
        super().__init__()
        assert isinstance(orig_layer, torch.nn.Linear), "orig_layer must be a torch.nn.Linear"
        ...
        self.scheme = scheme
        self.register_buffer('sq_scales', sq_scales)
        self.quantile = quantile

    def forward(self, input):
        # Check if sq_scales exists before applying it
        if self.sq_scales is not None:
            input = torch.mul(input, self.sq_scales)
        
        q_input = quant_weight(input, num_bits=self.bits, group_size=self.group_size,
                               scheme=self.scheme, data_type=self.dtype, quantile=self.quantile)
        return F.linear(q_input, self.weight, self.bias)
